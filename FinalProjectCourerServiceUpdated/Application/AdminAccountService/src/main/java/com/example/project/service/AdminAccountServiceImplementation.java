...
public class AdminAccountServiceImplementation implements AdminSignupService {
	...
	public APIResponse adminLogin(String username, String password) {
		...
		//System.out.print(adminDetailes.getRole());
		...
		if (adminDetailes != null) {
			if (adminDetailes.getPassword().equals(password)) {
				...
				return new APIResponse(200, "Successfull!", responseEntity.getBody(),adminDetailes.getRole());
				
			}
		}
		return new APIResponse(400, "Failed!", new AccessToken(),adminDetailes.getRole());
	}

	@Override
	public UserDetailes getUserByUsername(String username) {

		UserDetailes getdetailes = adminAccountRepository.findByUsername(username);
		return getdetailes;
	}
}
